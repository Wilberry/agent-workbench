      ...
      <nav className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8" aria-label="Main navigation">
        <div className={`rounded-2xl border px-4 py-3 backdrop-blur-xl transition ${scrolled ? 'border-white/10 bg-slate-950/90 shadow-2xl shadow-black/30' : 'border-white/[.08] bg-slate-950/65'}`}>
          <div className="flex items-center justify-between gap-4">
            <Link href="/#top" className="flex min-w-0 items-center gap-3 rounded-xl font-semibold text-white focus:outline-none focus:ring-2 focus:ring-cyan-300 focus:ring-offset-2 focus:ring-offset-slate-950">
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-cyan-300/20 bg-cyan-300/[.07] text-cyan-300"><Icon name="layers" /></span>
              <span className="truncate">Agent Workbench</span>
            </Link>
            ...
            <button
              ...
              className="rounded-xl border border-white/10 p-2 text-white transition hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-cyan-300 focus:ring-offset-2 focus:ring-offset-slate-950 md:hidden"
            >
          ...
          {open ? (
            <div id="mobile-navigation" className="mt-4 grid gap-1 border-t border-white/10 pt-4 md:hidden">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 border-t border-white/10 pt-4">
                <Link onClick={() => setOpen(false)} href={githubUrl} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-sm font-semibold text-white hover:bg-white/[.06] focus:outline-none focus:ring-2 focus:ring-cyan-300 focus:ring-offset-2 focus:ring-offset-slate-950">
                  <Icon name="github" /> GitHub
                </Link>
                <Link onClick={() => setOpen(false)} href="/signup" className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-semibold text-slate-950 hover:bg-cyan-100 focus:outline-none focus:ring-2 focus:ring-cyan-300 focus:ring-offset-2 focus:ring-offset-slate-950">
                  Get started <Icon name="arrow" className="h-4 w-4" />
                </Link>
              </div>
            </div>
