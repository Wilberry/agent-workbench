...
import { persistExecutionStep, markRunCompleted, markRunFailed, markQueueJobCompleted, markQueueJobFailed, incrementAttemptsAndMaybeDead, reclaimStaleJobs, updateRunTelemetry, setProcessing } from './queue';
...
function formatMemoryContext(memories = []) {
    ...
    return memories
        .map((memory) => `- ${memory.role === 'user' ? 'User said' : 'Assistant responded'}: ${memory.content} (Similarity: ${memory.similarity.toFixed(2)})`)
        .join('\n');
}
...
export async function processAgentRunJob(job) {
    const { runId, message, workflow, memories } = job;
    const supabase = createServerSupabaseClient();
    const persistEvent = async (eventType, payload) => {
    ...
    try {
        ...
        const existingTrace = existingRun?.execution_trace || [];
...
${JSON.stringify(toolResult, null, 2)}`
                    ...
                    if (retryAttempt < MAX_RETRIES) {
                        ...
                        console.log(`Step ${stepIndex} (${role}) failed, retrying in ${backoffMs}ms (attempt ${retryAttempt + 1}/${MAX_RETRIES})`);
