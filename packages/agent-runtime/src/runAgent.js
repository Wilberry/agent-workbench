...
function buildSystemPrompt(agentPrompt, tools, memoryContext) {
...
export async function runAgent({ agentId, conversationId, userMessage, debug = false, runId }) {
    ...
    const userEmbeddingPromise = generateEmbedding(userMessage)
        .then((vector) => supabase.from('messages').update({ embedding: vector }).eq('id', userMessageRow.id))
        .catch((error) => {
    ...
    let versionWorkflow = undefined;
    if (latestVersion) {
        if (latestVersion.system_prompt)
            selectedSystemPrompt = latestVersion.system_prompt;
